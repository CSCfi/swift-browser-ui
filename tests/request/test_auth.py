...
from swift_browser_ui.request.auth import read_in_keys, handle_validate_authentication
from swift_browser_ui.request.auth import test_signature as t_signature
...
class AuthModuleTestCase(asynctest.TestCase):
    ...
    async def test_read_in_keys(self):
        ...
        os_environ_patch = unittest.mock.patch(
            "swift_browser_ui.request.auth.os.environ.get", new=os_environ_mock
        )
    ...
    async def test_test_signature_success(self):
        ...
        await t_signature(tokens_mock, signature, message_mock, time.time() + 3600)
    ...
    async def test_handle_validate_authentication_success(self):
        ...
        t_signature_patch = unittest.mock.patch(
            "swift_browser_ui.request.auth.test_signature", t_singature_mock
        )

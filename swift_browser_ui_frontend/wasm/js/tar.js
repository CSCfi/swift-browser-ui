...
export function addTarFolder(name, prefix) {
  let mtime = Math.floor(Date.now() / 1000).toString();
  ...
  let header_start =
      ...
      + mtime.padEnd(12, "\x00") // Last modification, 11 bytes + padding nul
    ;

  // Calculate checksum in between to ease header calculation
  let checksum = 0;
  for(let i = 0, length = 148; i < length; i++) {
    checksum += header_start.charCodeAt(i);
  }

  let header =
      header_start
      + String(checksum).padEnd(8, "\x00")  // padded checksum
      ...
      + "\x00".repeat(16)  // skip device major and minor numbers
...
export function addTarFile(name, prefix, size) {
  ...
  let header_start =
      ...
      + size.toString(8).padEnd(12, "\x00")
      + mtime.padEnd(12, "\x00")  // Last modification, not used, 11 bytes + padding NUL
    ;
